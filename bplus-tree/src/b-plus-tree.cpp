...
void BPlusTree::insert(int key, int page_id) {
    ...
    if (current_node->keys.size() > this->node_cap) {
        ...
        right_node->values = std::vector<int>(
            current_node->values.begin() + mid, current_node->values.end());
        // overwrite keys for current node, current node -> leaf node
        current_node->keys = left_keys;

        current_node->values = std::vector<int>(
            current_node->values.begin(), current_node->values.begin() + mid);
        ...
        while (!path.empty()) {
            ...
            parent_node->children_nodes.insert(
                parent_node->children_nodes.begin() + child_idx + 1,
                right_node);
            ...
            right_node->keys = right_keys;
            right_node->children_nodes = std::vector<Node *>(
                parent_node->children_nodes.begin() + mid + 1,
                parent_node->children_nodes.end());

            parent_node->keys.erase(parent_node->keys.begin() + mid,
                                    parent_node->keys.end());

            parent_node->children_nodes.erase(
                parent_node->children_nodes.begin() + mid + 1,
                parent_node->children_nodes.end());
        }
