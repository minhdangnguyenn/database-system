...
int BufferPool::create_new_page() {
  ...
  if (this->free_frame_list.empty()) {
    bool is_evictable =
        this->replacer->evict(frame_id); // frame_id will be init here
    if (!is_evictable) {
...
char *BufferPool::fetch_page(int page_id) {
  ...
  // else if buffer pool has no free frame left -> evict one using replacer
    ...
    if (!can_evict) {
      std::cout << "Replacer can not evict frame " + std::to_string(frame_id)
                << std::endl;
      return nullptr;
    }
    ...
    std::cout << std::to_string(frame_id) + " has been successfully evicted !"
              << std::endl;
...
void BufferPool::flush_page(int page_id) {
  ...
  if (iter == this->page_table.end()) {
    std::cout << "PAGE ID" + std::to_string(page_id) +
                     "CANNOT FOUND WHILE FLUSHING PAGE"
              << std::endl;
  } else {
